        ...
        {[
          ...
          <div key={address.id} className="flex items-start justify-between p-4 border rounded-lg hover:border-iazi-primary transition-colors">
            <div className="space-y-1">
              <h3 className="font-playfair font-medium text-lg text-iazi-text">{address.title}</h3>
              <p className="font-lato text-muted-foreground">{address.street}</p>
              <p className="font-lato text-muted-foreground">
                {address.neighborhood}, {address.city} - {address.state}
              </p>
            </div>
            <div className="flex space-x-2">
              <Button size="icon" variant="outline" className="hover:text-iazi-primary hover:border-iazi-primary">
                <Edit className="h-4 w-4" />
              </Button>
              <Button size="icon" variant="outline" className="hover:text-red-500 hover:border-red-500">
